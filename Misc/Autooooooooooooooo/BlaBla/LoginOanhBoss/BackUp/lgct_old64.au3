...
Func Capt()
	...
	If Third("OtherKNB") Then
		Local $name = StringSplit($getDat[3], "_")
		$sv = $name[2]
	EndIf

	Local Const $sFilePath = "D:\Vang\Vang\" & $sv
;~ 	DirC()
	$hBmp = _ScreenCapture_CaptureWnd("", $WindowHandle, 205, 42, 223, 50)
...
_GDIPlus_Shutdown()
	...
	If FileExists($sFilePath) Then
		_ScreenCapture_SaveImage($sFilePath & "\" & $getDat[1] & ".jpg", $hBmp)
	Else
		DirCreate($sFilePath)
		_ScreenCapture_SaveImage($sFilePath & "\" & $getDat[1] & ".jpg", $hBmp)
