...
Func _ChonSV($WindowHandle, $SVer, $fileIni)
	...
	While 1
		...
		_ADB_CMD("input swipe 200 531 200 866 1000", $port)
		If StringInStr($SVer, "SV", 1) Then
			IniWrite($fileIni, "General", "SV", $SVer)
...
Func _imgSearch($WindowHandle, $SVer, $top = 0, $left = 0, $bot = -1, $right = -1)
	...
	While 1
		Sleep(10)
		$Result = _HandleImgSearch($WindowHandle, @ScriptDir & "\Data\Images\" & $SVer & ".bmp", $top, $left, $bot, $right, 59, 100)
		If Not @error Then
			Sleep(10)
			$x = $Result[1][0]
			$y = $Result[1][1]
...
Func _Wait4Image($WindowHandle, $bmp)
	...
	While 1
		...
		If Not @error Then
			Sleep(100)
			ControlClick($WindowHandle, "", "", "left", 1, $Result[1][0], $Result[1][1] + 31)
;~ 			Sleep(200)
