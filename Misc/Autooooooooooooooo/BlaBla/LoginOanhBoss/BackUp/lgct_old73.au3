...
;~ 		MsgBox(0, "", "60S SHUT2222DƠN", 60)
		...
		If WinExists("MEmu - ") = 0 Then
			MsgBox(0, "", "60S SHUTDƠN", 60)
			Sleep(70000)
			Shutdown(12)
		EndIf
	EndIf
...
;~ 	Mbox("asdfs")
	...
	_ClickUtilSeeImgNoSleep($WindowHandle, "iconBlueDTK", 33, 190)
...
;~ 	khu vuc sung bai bxh
	If Third($infoAcc, "9t") or IniRead($iniFile, "General", "SV", 0) = "SV283" Or IniRead($iniFile, "General", "SV", 0) = "SV341" Or IniRead($iniFile, "General", "SV", 0) = "SV338" Or IniRead($iniFile, "General", "SV", 0) = "SV555" Or IniRead($iniFile, "General", "SV", 0) = "SV487" Or IniRead($iniFile, "General", "SV", 0) = "SV574" Or IniRead($iniFile, "General", "SV", 0) = "SV281" Or IniRead($iniFile, "General", "SV", 0) = "SV284" Then
...
;~ 	Mbox($grsv)
;~ 	Run(@ComSpec & ' /c adb -s 127.0.0.1:5557 shell input swipe 200 860 200 500 100 ', "", @SW_HIDE)
;~ 	Sleep(500)
;~ 	Run(@ComSpec & ' /c adb -s 127.0.0.1:5557 shell input swipe 200 860 200 500 100 ', "", @SW_HIDE)
;~ 	Sleep(500)
;~ 	Run(@ComSpec & ' /c adb -s 127.0.0.1:5557 shell input swipe 200 860 200 500 100 ', "", @SW_HIDE)
;~ 	Sleep(500)
	While 1
		...
		While 1
			...
			If _imgSearch($WindowHandle, "S" & $grsv) = 1 Or _imgSearch($WindowHandle, $SVerfile) = 1 Then
				Sleep(400)
;~ 				MsgBox(0,82-$x,197+$y)
;~ 				ControlClick($WindowHandle, "", "", "left", 1,$x, $y-31)

				_CtrlClick($WindowHandle, Number($x), Number($y))
;~ 				_CtrlClick($WindowHandle,Number($x), Number(281-32))
;~ 				_CtrlClick($WindowHandle,Number($x), Number(281-32))
				MouseMove($x, $y)
;~ 				MsgBox(0,$x,$y)
				Return
...
;~ 			If _imgSearch($WindowHandle, "S181",60, 197,82, 342) = 1
		...
		WEnd
		Sleep(100)
		_ADB_CMD("input swipe 200 531 200 866 2000", $port)
		If StringInStr($SVer, "SV", 1) Then
			IniWrite($fileIni, "General", "SV", $SVer)
