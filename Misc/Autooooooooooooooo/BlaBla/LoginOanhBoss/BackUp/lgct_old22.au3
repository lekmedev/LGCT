...
MsgBox(0,"",_CoProc("AutoMain", $WindowHandle[1][1]))
...
Func AutoMain($WindowHandle = Default)
	...
	IsHwnd($WindowHandle) then
	if MsgBox(0, "", $WindowHandle) then
	endif
...
Func _Wait4Image($WindowHandle, $bmp)
	...
	While 1
		Local $Result
		$Result = _HandleImgSearch($WindowHandle, @ScriptDir & "\Data\Images\" & $bmp & ".bmp", 0, 0, -1, -1, 40, 100)
		If Not @error Then
			Sleep(1100)
			MsgBox(0, "", "", 2)
			ControlClick($WindowHandle, "", "", "left", 1, $Result[1][0], $Result[1][1])
			ControlClick($WindowHandle, "", "", "left", 1, $Result[1][0], $Result[1][1])
			_CtrlClick($WindowHandle, $Result)
...
Func _CtrlClick($WindowHandle, $Result)
	ControlClick($WindowHandle, "", "", "left", 1, $Result[1][0], $Result[1][1])
	Sleep(100)
	ControlClick($WindowHandle, "", "", "left", 1, $Result[1][0], $Result[1][1])
EndFunc   ;==>_CtrlClick
