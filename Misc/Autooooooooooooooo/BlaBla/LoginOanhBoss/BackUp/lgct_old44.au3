...
;~ 			OKIF($WindowHandle, "iconListServer")
			ChonSV($infoAcc[1],$fileIni,$WindowHandle,$port)
...
Func ChonSV($SVer, $fileIni, $WindowHandle,$port)
	...
	While 1
		...
		Local $Result
		$Result = _HandleImgSearch($WindowHandle, @ScriptDir & "\Data\Images\" & $SVer & ".bmp", 0, 0, -1, -1, 10, 100)
;~ 		$Result = _HandleImgSearch($WindowHandle, @ScriptDir & "\Data\Images\" & _ReadIni($fileIni, "General", "SV", 1) & ".bmp", 0, 0, -1, -1, 10, 100)
		if (_ReadIni($fileIni, "General", "SV", 1) = 1) Then
		...
		EndIf
		Return
		_ADB_CMD("input swipe 546 416 546 327 1000", $port)
