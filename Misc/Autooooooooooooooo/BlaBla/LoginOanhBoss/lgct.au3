...
Global $port, $i, $x = 0, $y = 0, $WindowHandle, $tenTK, $iniFile, $iGlobal, $aPos, $aPoss, $rsMod, $sFilePathvang

Func AutoMain()
	Local $a = 0, $infoAcc, $samePWD
	AdlibRegister("_ClearApp", 30000)
	_CtrlClick(136, 62)
	Local $fileList = @ScriptDir & "\ListAcc.txt"
	...
	For $i = 1 To _FileCountLines($fileList)
		...
		$tenTK = $infoAcc[1]
		Sleep(100)
		_ClickUtilSeeImg("iconDay", 133, 279, 100)
;~ 		----------------------------------------------------------------------------------------------------------- Change SV

		If StringInStr($infoAcc[1], "SV", 1) = 1 Then
			_OKIF($WindowHandle, "iconDay")
			Sleep(300)
			_ClickUtilSeeImg("iconListServer", 91, 349)
			AdlibRegister("_ClearApp", 60000)
			_ChonSV($WindowHandle, $infoAcc[1])
			ContinueLoop
		EndIf
...
Func _ThinhAn($WindowHandle, $infoAcc)
	...
	_ClickUtilSeeImg("iconDaNhan", 228, 489)
...
;~ 	EndIfbig_daisy083
	Run(@ComSpec & ' /c ""C:\Program Files\ImageMagick-7.0.10-Q16\magick" convert "E:\Temp\LDPhoTo\' & $infoAcc[1] & '.jpg" -crop 65x20+520+110  "D:\Vang\Vang\' & _ReadIni($iniFile, "General", "SV", 0) & '\' & $infoAcc[1] & '.jpg""', "", @SW_HIDE)
EndFunc   ;==>_ThinhAn
...
Func DoiCode()
	...
	_ClickUtilSeeImg("iconDoiCode", 24, 409)
	Sleep(29)
...
;~ 	Mbox($code)
	Sleep(10)
	_CtrlClick(136, 271)
	_ClickUtilSeeImg("iconDoiCode", 243, 184)
	_CtrlClick(243, 98)
EndFunc   ;==>DoiCode

Func _ChonSV($WindowHandle, $SVer)

	Local $SVtrimmed = StringTrimLeft($SVer, 2)


	AdlibRegister("_ClearApp", 130000)
	Local $grsv = GrSV($SVtrimmed)
