...
;~ 720x1280 320dpi
;~ _Singleton(
Global $port, $x = 0, $y = 0, $tenTK, $iniFile, $aPos, $aPoss, $rsMod, $sFilePathvang, $render_hwnd, $WindowHandle, $flag = 1, $motiec = 0, $flgMT = 0, $iGlobal , $fkexit=0
...
;~ 		MsgBox(0,"","")
		...
		Local $infoAcc, $samePWD = "null", $sum = _FileCountLines($fileList) + 1
...
Func clone($infoAcc, $samePWD)
	...
	_CtrlClickInGame(33, 366)
	Sleep(1000)
;~ 	_CtrlClickInGame(141, 262)

;~ 	Mbox("doneBD")
;~ 	Do
;~ 		_CtrlClickInGame(134, 273)
;~ 		Sleep(100)
;~ 		Mbox("done123123")
;~ 	Until _imgSearch("cloneINBD") = 1
;~ 	Mbox("doneBD2")
;~ 	While 1
;~ 		_CtrlClickInGame(154, 356)
;~ 		Sleep(100)
;~ 		Mbox("1")
;~ 		If _imgSearch("cloneINBD") = 1 Then
;~ 			ExitLoop
;~ 		EndIf
;~ 	WEnd


	AdlibRegister("_checkTQ",14000)
...
;~ 	ControlSend($WindowHandle, "", "", "bán clone 10k 1 acc (còn 200 con), nhận sb thỉnh an. Zalo 094.775.9965")
	ControlSend($WindowHandle, "", "", "Nhận Buff 200k điểm tiệc = 300k VND, Zalo 094.775.9965")
	Sleep(3000)
;~ 	Mbox("1")
