...
;~ 		--------------------------------------------------------------------------------------------------------
		...
		If ProcessExists(WinGetProcess($WindowHandle)) And $flag = 1 Then
			...
			If WinExists("[CLASS:LDPlayerMainFrame]") = 0 Then
				Sleep(3000)
				RunWait(@ComSpec & ' /c RUNDLL32.EXE powrprof.dll,SetSuspendState 0,1,0', "", @SW_HIDE)
			...
			If @HOUR < 22 Then
				...
				If Third($infoAcc, "Prison") Then
					If Mod(@MDAY, 2) <> 0 Then
						_Prison()
					EndIf
				ElseIf Third($infoAcc, "Pri2son") Or _curSV() = "SV283" Or _curSV() = "SV280" Then
					_Prison()
				EndIf
			EndIf
...
;~ 	_CtrlClickInGame(248, 40) ;delete here
;~ 	---------------
...
;~ 	$Phu
	Switch StringTrimLeft(_curSV(), 2)
		...
		Case 396
			$Phu = $_Van_Tuyen
		Case 407, 586, 279, 284, 287, 278, 530, 502, 109, 111, 193
			$Phu = $_Binh_Tay
		Case 381, 182, 380, 433, 408, 148, 149, 240
			$Phu = $_Tran_Nam
		Case 461
			$Phu = $_Bac_Tinh
		Case 415
