...
Func ADB()
	...
	Run($Nox_Path & "\adb.exe devices", $Nox_Path, @SW_SHOW)
	MsgBox(0, '', 'Pause')

	Global $Nox_Path = "D:\Program Files\Nox\bin"
$DOS = Run($Nox_Path & "\nox_adb.exe devices", $Nox_Path, @SW_SHOW, $STDERR_MERGED)
While 1
    Sleep(50)
...
Func AutoMain()
	...
	_Wait4Image($WindowHandle, "lgct", "day")

EndFunc   ;==>AutoMain

Func _Wait4Image($WindowHandle, $bmp, $nextbmp)
	While 1
		Local $Result
