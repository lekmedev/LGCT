...
;~ 			MsgBox(0, "", "SVday", 1)
	...
	_CtrlClickInGame($WindowHandle, 251, 58)
...
;~ 			If _imgSearch($WindowHandle, "S181",60, 197,82, 342) = 1
		...
		Sleep(100)
;~
		_ADB_CMD("input swipe 251 800 251 522 1500", $port)
;~ 		846.5 522
		If StringInStr($SVer, "SV", 1) Then
			IniWrite($fileIni, "General", "SV", $SVer)
...
Func _normalSearch($SVtrimmed)
	If _imgSearch($WindowHandle, "461", 64, 232, 84 - 64, 374 - 232) = 1 Then
				$x=$x+64
				$y=$y+232
				MouseMove($x,$y)
				Return 1
...
;~ 		Local $a1 = TimerInit()
;~ 		While 1
;~ 			If _imgSearch($WindowHandle, "481", 64, 232, 84 - 64, 374 - 232) = 1 Then
;~ 				$x=$x+64
;~ 				$y=$y+232
;~ 				MouseMove($x,$y)
;~ 				Return 1
;~ 			EndIf
;~ 			If TimerDiff($a1) > 5000 Then
;~ 			Return
;~ 		EndIf
;~ 		WEnd
