...
;~ 720x1280 320dpi
;~ _Singleton(
Global $port, $x = 0, $y = 0, $Phu, $tenTK, $iniFile, $aPos, $aPoss, $rsMod, $sFilePathvang, $render_hwnd, $WindowHandle, $flag = 1, $motiec = 0, $flgMT = 0, $iGlobal, $fkexit = 0, $iGlobal1, $sv, $iVPN
...
;~ Return
;~ 	 Mbox($iVPN)
	Run(@ComSpec & ' /c adb -s 127.0.0.1:' & $port & ' shell am start -a android.intent.action.MAIN -n de.blinkt.openvpn/.LaunchVPN -e de.blinkt.openvpn.shortcutProfileName NordVpn_' & $iVPN & '', @SystemDir, @SW_HIDE)
	Sleep(20000)
	$flag = 0
;~ Run(@ComSpec & ' /c adb -s 127.0.0.1:' & $port & ' shell monkey -p com.hd.vngjyp -c android.intent.category.LAUNCHER 1', @SystemDir, @SW_HIDE)

	$iVPN += 1
	_WriteIni($iniFile, "General", "BienIVPN", $iVPN)
EndFunc   ;==>_vpn
...
;~ 	Run(@ComSpec & ' /c adb -s 127.0.0.1:' & $port & ' shell am start -a android.intent.action.MAIN -n de.blinkt.openvpn/.LaunchVPN -e de.blinkt.openvpn.shortcutProfileName NordVpn_', @SystemDir, @SW_HIDE)
;~ 	Sleep(34000)
	Mbox("OpenVPN",30)
...
;~ 			If _curSV() = "SV284" or Third($infoAcc, "MoBoss")  Or _curSV() = "SV283" Or _curSV() = "SV280" Then
;~ 				_Mail()
;~ 			EndIf
			...
			while 1
				Sleep(100)
				if TimerDiff($startT) > 50000 Then
					Mbox("1 Phut")
					ExitLoop
			EndIf
				WEnd
			ExitToLog()
			AdlibRegister("_checkFB", 3000)
;~
		...
		If ProcessExists(WinGetProcess($WindowHandle)) And $flag = 1 Then
			...
			If WinGetTitle($WindowHandle) = "2" Then
				Run(@ComSpec & ' /c adb -s 127.0.0.1:' & $port & ' shell am force-stop com.vnxldj.vn', "", @SW_HIDE)
				Sleep(3000)
				Run(@ComSpec & ' /c adb -s 127.0.0.1:' & $port & ' shell input keyevent 3', "", @SW_HIDE)
				ShellExecuteWait("E:\App\AutoLGCTLDPlayer - Tan Loc Dinh Ky\Net2.au3")
				Exit
			EndIf
...
;~ 		--------------------------------------------------------------------------------------------------------
		...
		If ProcessExists(WinGetProcess($WindowHandle)) And $flag = 1 Then
			...
			ShellExecuteWait(@ScriptDir & "\war" & WinGetTitle($WindowHandle) & ".au3")
			Exit
...
;~ 			MsgBox(0, "", "SVday", 1)
		EndIf
	Until _imgSearch("iconPopupTemp2") = 1 ;delete here
;~ 	Until _imgSearch("iconPopup", 251, 148, 262 - 251, 160 - 148) = 1

	AdlibRegister("_ClearApp", 20000)
	Sleep(100) ;delete here
	_CtrlClickInGame(256, 31, 5) ;delete here
;~ 	Mbox("1")

;~ 	_ADB_CMD("input tap 644,77", $port)q
;~ 	_ClickUtilSeeImg("iconPopup",245, 35)
;~ 	---------------
...
;~ 	If _curSV() = "SV284" Or Third($infoAcc, "MoBoss") Or Third($infoAcc, "SVer287") Or _curSV() = "SV278"  Or _curSV() = "SV283"  Or _curSV() = "SV283" Then
;~ 		tem()
;~ 		_OKIF("iconTrangTri")
;~ 		AdlibRegister("_ClearApp", 30000)
;~ 	EndIf
...
Switch StringTrimLeft(_curSV(), 2)
		...
		Case 407, 586, 530, 502, 109, 111, 193, 283
			$Phu = $_Binh_Tay
		Case 381, 182, 380, 433, 408, 148, 149
			$Phu = $_Tran_Nam
		Case 4611
			$Phu = $_Bac_Tinh
		Case 415
...
;~ 	khu vuc sung bai bxh
	If Third($infoAcc, "9t") Or IniRead($iniFile, "General", "SV", 0) = "SV468" Or IniRead($iniFile, "General", "SV", 0) = "SV280" Or IniRead($iniFile, "General", "SV", 0) = "SV596" Or IniRead($iniFile, "General", "SV", 0) = "SV472" Or IniRead($iniFile, "General", "SV", 0) = "SV283" Or IniRead($iniFile, "General", "SV", 0) = "SV284" Or IniRead($iniFile, "General", "SV", 0) = "SV16" Or IniRead($iniFile, "General", "SV", 0) = "SV185" Or IniRead($iniFile, "General", "SV", 0) = "SV186" Then
...
;~ 	mbox((100-((Int(StringTrimLeft(_curSV(), 2)/50))*8))*1000)
	AdlibRegister("_ClearApp", ((11.3 - (Ceiling(Int(StringTrimLeft(_curSV(), 2) / 50)))) * 10000 * 0.7))
...
Func _ResetNewDay($fileIni)
	If _ReadIni($fileIni, "General", "Day", 0) <> @MDAY Then
		...
		_WriteIni($fileIni, "General", "BienIVPN", 31)
