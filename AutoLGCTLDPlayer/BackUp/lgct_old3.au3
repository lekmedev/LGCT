...
;~ 720x1280 320dpi
;~ _Singleton(
Global $port, $iniFileToTiec, $countTime, $countBoss = 1, $x = 0, $y = 0, $Phu, $tenTK, $iniFile, $aPos, $aPoss, $rsMod, $sFilePathvang, $render_hwnd, $WindowHandle, $flag = 1, $motiec = 0, $flgMT = 0, $iGlobal, $fkexit = 0, $iGlobal1, $sv, $iVPN

;~ WinGetTitle($WindowHandle)
Func ClickSeeDay_Backup()
...
Func ClickSeeDay()
	...
	If _imgSearch("iconFB") = 1 Then
		...
		Local $startTime = TimerInit()
			...
			Sleep(300)
			_CtrlClickInGame(200, 266)
		While 1
			Sleep(50)
			If $flag = 0 Then Return
;~ Mbox('1')
			If _imgSearch("iconDay", 234, 422, 281, 422) = 1 or _imgSearch("iconDay1", 234, 422, 281, 422) = 1 Then
				ExitLoop
			ElseIf _imgSearch("iconMoi", 238, 422, 278, 422) or _imgSearch("iconMoi1", 238, 422, 278, 422) = 1 Then
				ExitLoop
			EndIf
			If TimerDiff($startTime) > 10000 Then
				Return
...
Func DDMN()
	For $i = 0 To 4
		_CtrlClickInGame(31 + ($i * 53), 163)
		Sleep(10)
	Next
	For $i = 0 To 4
		_CtrlClickInGame(31 + ($i * 53), 355)
		Sleep(10)
	Next
EndFunc   ;==>DDMN

Func QuaHG()
	If _imgSearch("quaHG") = 1 Then
		Mbox('1')
		_ClickUtilSeeImg("inNQ", $x - 20, $y - 36)
	EndIf
...
;~ 	Mbox("1")
	...
	Sleep(1000)
	_ClickUtilSeeImg("iconInBang", 210, 274, 500)
;~ 	_OKIF("iconInBang")
	Sleep(500)
;~ 	-------------------- van tieu
...
;~ 	Mbox("1")
	...
	Sleep(1000)
	If _imgSearchW("inDi", 1000) Or _imgSearchW("chienVT", 1000) Then
		Return
	EndIf
;~ 	Mbox("1")
...
;~ 	Mbox("1")
	...
	If @MDAY = 24 And Third($infoAcc, "qgia") = False Then
...
Func _TVien()
	...
	_ClickUtilSeeImg("GL", 261, 339, 1500) ; click thầy
	Sleep(400)
;~ 	Mbox("1")
	_CtrlClickInGame(217, 486)
	Sleep(2000)
;~ 	_CtrlClickInGame(253, 15)
	_ClickUtilSeeImg("iconTrangTri", 253, 15, 1100) ; click thầyMbox("1")
;~ 	_OKIF("iconTrangTri")
EndFunc   ;==>_TVien
			...
			If _imgSearch("okPrison") = 0 Then
				...
				While 1
					...
					ElseIf _imgSearch("bossDie") Then
						Return
					ElseIf _imgSearch("diethet") Then
						Return
...
;~ 			Mbox($iGlobal)
	...
	AdlibRegister("_checkCountTime", 180000)
	While 1
		Sleep(50)
		If $flag = 0 Then
			$flag = 1
...
;~ 			$iGlobal1 = $iGlobal11
			...
			ElseIf $infoAcc[2] == 'nullll' Or _curSV() == 'SV593' Then
				$iGlobal = $iGlobal + 1
				_WriteIni($iniFile, "General", "BienI", $iGlobal)
				ContinueLoop
			EndIf
...
;~ 			============================================= Vpn
			...
			If _ReadIni(@ScriptDir & "\Data\Account\file.ini", "General", "iVPN", 1) > 40 Then
				Sleep(10000)
...
Func _muaMat()
	Opt("WinTitleMatchMode", 3)
	Mbox("Mua Mat", 99999999999999999999)
	If _Singleton("AutoLDPlayer" & WinGetTitle($WindowHandle), 1) = 0 Then
		MsgBox($MB_SYSTEMMODAL, "Warning", "An occurrence of test is already running", 1)
		Exit
	EndIf
	$countTime = TimerInit()
	Local $fileIni = @ScriptDir & "\Data\Account\Mat\file" & WinGetTitle($WindowHandle) & ".ini", $fileList = @ScriptDir & "\Data\Account\Mat\AutoList" & WinGetTitle($WindowHandle) & ".txt"
...
;~ 			$iGlobal1 = $iGlobal11
			...
			If StringInStr($infoAcc[1], "SV", 1) And $flgMT <> 2 Then
				IniWrite($fileIni, "General", "SV", $infoAcc[1])
			ElseIf $infoAcc[2] == 'saicmnpasss' Or $infoAcc[2] == 'null' Or $infoAcc[2] == '06202000' Or $infoAcc[2] == 'xiaomeng' Then
				$iGlobal = $iGlobal + 1
				_WriteIni($iniFile, "General", "BienI", $iGlobal)
				ContinueLoop
			EndIf
...
;~ ------------------------------- Mat bang
			Sleep(1500)
			_ClickUtilSeeImg("iconCho", 280, 24, 2000)

			_Wait4Image("iconCho")
			Sleep(1000)
			_CtrlClickInGame(96, 81)
			Sleep(1000)
			AdlibRegister("_ClearApp", 20000)
			_ADB_CMD("input swipe 100 1115 100 810 1000", $port)
			_ADB_CMD("input tap 100 810", $port)
			Sleep(500)

			While 1
				Sleep(150)
				If $flag = 0 Then ExitLoop
				_CtrlClickInGame(225, 428)
				If $flag = 0 Then ExitLoop
				If _imgSearch("max") = 1 Then
					ExitLoop
				ElseIf _imgSearch("maxKNB") = 1 Then
					ExitLoop
				EndIf
			WEnd
...
;~ 			============================================= Vpn
			...
			AdlibRegister("_ClearApp", 40000)
			_WriteIni(@ScriptDir & "\Data\Account\file.ini", "General", "iVPN", 1 + _ReadIni(@ScriptDir & "\Data\Account\file.ini", "General", "iVPN", 1))
			If _ReadIni(@ScriptDir & "\Data\Account\file.ini", "General", "iVPN", 1) > 60 Then
				Sleep(30000)
...
;~ 			Mbox($iGlobal)
	...
	AdlibRegister("_checkCountTime", 120000)
...
;~ 			ElseIf $infoAcc[2] == 'saicmnpasss' Then
			ElseIf $infoAcc[2] == 'saicmnpasss' Or $infoAcc[2] == 'xiaomeng' Then
				$iGlobal = $iGlobal + 1
				_WriteIni($iniFile, "General", "BienI", $iGlobal)
				ContinueLoop
			EndIf
...
;~ 			If (_curSV() = "SV284" Or Third($infoAcc, "MoBoss") Or Third($infoAcc, "SV104") Or Third($infoAcc, "SV163") Or _curSV() = "SV287" Or _curSV() = "SV279" Or _curSV() = "SV502" Or _curSV() = "SV278") Then
;~ 				_Mail()
;~ 			EndIf;~

			If (_curSV() = "SV271" Or _curSV() = "SV269" Or _curSV() = "SV284" Or Third($infoAcc, "MoBoss") Or Third($infoAcc, "CheckMail") Or Third($infoAcc, "SV104") Or Third($infoAcc, "SV163") Or _curSV() = "SV287" Or _curSV() = "SV279" Or _curSV() = "SV502" Or _curSV() = "SV278") And (@MDAY = 19 Or @MDAY = 4) Then
				_Mail()
			EndIf
...
;~ 				EndIf
			...
			If Third($infoAcc, "100V") And Mod(@MDAY, 2) <> 0 Then
				_CHBang($infoAcc)
			ElseIf Third($infoAcc, "XDBANG") Then
				_XDBANG()
			EndIf
...
;~ 	EndIf
	...
	If TimerDiff($countTime) > 250000 Then
		$countTime = TimerInit()
;~ 		mbox(TimerDiff($countTime))
		_WriteIni(@ScriptDir & "\Data\Account\file.ini", "General", "runVPN", 1)
;~ 		_WriteIni(@ScriptDir & "\Data\Account\file.ini", "General", "iVPN", 0)
;~ 		$countTime = TimerInit()
;~ 		Sleep(5000)
;~ 		ShellExecute(@ScriptDir & "/ChangeProxy.au3")
...
Func AutoMainVanTieu()
	...
	If @HOUR > 14 Then
		_WriteIni($fileIni, "General", "Day", 33)
	EndIf
	_ResetNewDay($fileIni)
	Local $infoAcc, $samePWD, $sum = _FileCountLines($fileList) + 1
...
;~ 			============================================= Vpn
			...
			AdlibRegister("_ClearApp", 40000)
			_WriteIni(@ScriptDir & "\Data\Account\file.ini", "General", "iVPN", 1 + _ReadIni(@ScriptDir & "\Data\Account\file.ini", "General", "iVPN", 1))
			If _ReadIni(@ScriptDir & "\Data\Account\file.ini", "General", "iVPN", 1) > 55 Then
				Sleep(30000)
...
;~ 			ShellExecute(@ScriptDir & " - Net\.Net" & WinGetTitle($WindowHandle) & " - Copy.au3")
;~ 			Exit
...
;~ 			Mbox("1")
		...
		While 1
			...
			If _imgSearchW("haveEmail", 1500) = 1 Or _imgSearchW("haveEmail2", 1500) = 1 Then
				...
				Sleep(1000)
				_CtrlClickInGame(55, 264)
				Sleep(1000)
				_CtrlClickInGame(258, 82)
				_OKIF("inEmail")
			ElseIf _imgSearchW("haveEmail3", 1500) = 1 Or _imgSearchW("haveEmail4", 1500) = 1 Then
				AdlibRegister("_ClearApp", 10000)
				_CtrlClickInGame($x, $y)
				Sleep(1000)
				_CtrlClickInGame(258, 82)
...
Func _XDBANG()
	...
	_OKIF("iconInBang")
	Sleep(300)
	If _imgSearch("pbang") = 1 Then
		_CtrlClickInGame(114, 367)
	Else
...
Func _Bchien()
	...
	_CtrlClickInGame(67, 409)
	Sleep(1500)
	_ClickUtilSeeImg("inCuBc", 213, 259, 200) ;740
;~ 	mbox("1")
	Sleep(500)
...
;~ 	if chua vBang---------------------------------
	...
	While 1
		Sleep(50)
		If $flag = 0 Then Return
		If _imgSearch("bangfull", 179, 275, 200 - 179, 295 - 275) = 1 Or _imgSearch("kChoVao") = 1 Then
;~ 		Mbox("123")
...
;~ 	EndIf
	...
	If @HOUR <> 20 Then
		Sleep(500)
		If Third($infoAcc, "GL") And @HOUR <> 12 Then
			AdlibRegister("_ClearApp", 10000)
			_TVien()
		EndIf
		...
		EndIf
	EndIf
	If (_curSV() = "SV284" Or _curSV() = "SV279") And @HOUR = 20 Then
		AdlibRegister("_ClearApp", 14000)
		QuanGia()
		_OKIF("iconTrangTri")
	EndIf
...
;~ 	AdlibRegister("repeatClickHC", 6000)
	...
	Switch StringTrimLeft(_curSV(), 2)
		...
		Case 396
			$Phu = $_Van_Tuyen
		Case 407, 586, 530, 502, 109, 111, 193
			$Phu = $_Binh_Tay
		Case 381, 182, 380, 433, 408, 416, 417, 148, 149, 283, 282, 281, 280, 528
			$Phu = $_Tran_Nam
		Case 407, 408
			$Phu = $_Than_Vuong
		Case 415
...
;~ 	khu vuc sung bai bxh
	If Third($infoAcc, "9t") Or IniRead($iniFile, "General", "SV", 0) = "SV472" Or IniRead($iniFile, "General", "SV", 0) = "SV468" Or IniRead($iniFile, "General", "SV", 0) = "SV402" Or IniRead($iniFile, "General", "SV", 0) = "SV406" Or IniRead($iniFile, "General", "SV", 0) = "SV280" Or IniRead($iniFile, "General", "SV", 0) = "SV282" Or IniRead($iniFile, "General", "SV", 0) = "SV472" Or IniRead($iniFile, "General", "SV", 0) = "SV283" Or IniRead($iniFile, "General", "SV", 0) = "SV28141" Then
...
Func _checkPWD()
	If _imgSearch("iconSaiMK", 184, 422 + 38, 26, 23) = 1 Or _imgSearch("iconTKKT") = 1 Or _imgSearch("loginFre") Then
...
;~ 		_imgSearch("fullqg.bmp")
		If _imgSearch("fullqg") = 1 Or _imgSearch("cmk") = 1 Then
			Sleep(300)
			_CtrlClickInGame(253, 15)
...
Func _AnTiec()
	If _Singleton(WinGetTitle($WindowHandle), 1) = 0 Then
		...
		Exit
	EndIf
	Mbox($iniFileToTiec, 5)
	Local $fileIni = @ScriptDir & "\Data\Account\Tiec\" & $iniFileToTiec & ".ini", $fileList = @ScriptDir & "\Data\Account\Tiec\" & $iniFileToTiec & ".txt"
...
;~ 		----------------------------------------------------------------------------------------------------------- Change SV
			...
			ElseIf StringInStr($infoAcc[1], "SV", 1) Or _ReadIni($fileIni, "General", "SVDAY", 1) = 1 Or $flgMT = 2 Then
...
;~ 	ControlSend($WindowHandle, "", "", StringTrimRight(@ScriptName,4))
	Sleep(333)
...
;~ 	_ClickUtilSeeImg("searchTiec", 187, 190, 1000)
	...
	If IniRead($iniFile, "General", "TK", 0) <> "NO" Then
		If _imgSearch("fullNho") Or _imgSearch("donetiec") = 1 Then
			$motiec = 1
			Return
		EndIf
	EndIf
...
;~ 	_Wait4Image("xntiec")
	While 1
		...
		If _imgSearch("xntiec") Then
			...
			_CtrlClickInGame(139, 89)
			Sleep(2000)
;~ 			mbox(IniRead($iniFile, "General", "MSDT", 0))
			If IniRead($iniFile, "General", "MSDT", 0) = "502000264" Then
				_CtrlClickInGame(224, 342)
			Else
