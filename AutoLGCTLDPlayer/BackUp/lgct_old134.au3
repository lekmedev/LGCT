...
;~ 720x1280 320dpi
;~ _Singleton(
Global $port, $x = 0, $y = 0, $WindowHandle, $tenTK, $iniFile, $iGlobal, $iGlobal1, $aPos, $aPoss, $rsMod, $sFilePathvang, $SUB_LDPLAYER_HWND = DllStructCreate("hwnd"), $render_hwnd, $DOS
...
Func _Prison()
	...
	While 1  ;click ulti ud danh ko du
		...
		_CtrlClickInGame($WindowHandle, 144, 300)
;~ 		Sleep(500)
		If _imgSearch($WindowHandle, "donePrison", 177, 280, 186 - 177, 293 - 280) = 1 Or _imgSearch($WindowHandle, "donePrison2", 177, 279, 189 - 177, 294 - 280) = 1 Then
			ExitLoop
		EndIf
	WEnd
EndFunc   ;==>_Prison
...
;~ 			ExitLoop
		...
		EndIf
	WEnd
	_CtrlClickInGame($WindowHandle, 245, 53)
	Sleep(500)
	_CtrlClickInGame($WindowHandle, 256, 19)
...
Func NameClone()
	_OKIF($WindowHandle, "iconXX")
	_CtrlClickInGame($WindowHandle, 204, 415)
	Sleep(100)
	_CtrlClickInGame($WindowHandle, 204, 415)
	Sleep(800)
	_CtrlClickInGame($WindowHandle, 155, 477)
	_CtrlClickInGame($WindowHandle, 155, 477)
	_CtrlClickInGame($WindowHandle, 155, 477)
	_OKIF($WindowHandle, "iconNext")
EndFunc   ;==>NameClone

Func TanThuClone()
	_ClickUtilSeeImg($WindowHandle, "TThoaAn", 135, 238)
	_ClickUtilSeeImg($WindowHandle, "TTKinhNong", 38, 345)
...
;~ 	_OKIF($WindowHandle, "iconDay")
	...
	While 1
		If _imgSearch($WindowHandle, "iconDay") = 1 Or _imgSearch($WindowHandle, "iconMoi") = 1 Then
			ExitLoop
		EndIf
	WEnd
...
;~ 		ControlSend($WindowHandle, "", "", $infoAcc[2])
		...
		_CtrlClickInGame($WindowHandle, 205, 228)
		Sleep(200)
		ControlSend($WindowHandle, "", "", "{BS 39}")
...
;~ 			MsgBox(0, "", "SVday", 1)
		...
		If TimerDiff($a) > 7000 Then
			If _imgSearch($WindowHandle, "iconPopupTemp") = 1 Then
				_CtrlClickInGame($WindowHandle, $x, $y)
			EndIf
			ExitLoop
		EndIf
	Until _imgSearch($WindowHandle, "iconPopupTemp") = 1
	Sleep(100)
...
;~ 	---------------
	_OKIF($WindowHandle, "iconPopup")
	Sleep(100)
	If _imgSearch($WindowHandle, "iconPopup", 251, 148, 262 - 251, 160 - 148) = 1 Then
;~ 		Mbox("See")
		_CtrlClick($WindowHandle, $x + 251, $y + 148)
	EndIf ; Xoa Temp
;~ 	---------------
...
Func ClickAgain()
	If _imgSearch($WindowHandle, "iconPopup") = 1 Then
		_CtrlClick($WindowHandle, $x, $y)
	EndIf
	AdlibUnRegister("ClickAgain")
EndFunc   ;==>ClickAgain
...
Func _Wait4Image($WindowHandle, $bmp, $top = 0, $left = 0, $bot = -1, $right = -1)
	Sleep(200)
	While 1
...
Func QuanGia()
	...
	If _imgSearch($WindowHandle, "iconGL", 249, 406 + 36, 273, 463) = 1 Then
		Return

	elseif _imgSearch($WindowHandle, "iconQG", 249, 406 + 36, 273, 463) = 1 Then
...
Func repeatClickHC()
	_CtrlClickInGame($WindowHandle, 207, 193)
	_CtrlClickInGame($WindowHandle, 170, 212)
;~ 	AdlibUnRegister("repeatClickHC")
