...
;~ 		--------------------------------------------------------------------------------------------------------
		...
		If ProcessExists(WinGetProcess($WindowHandle)) And $flag = 1 Then
			...
			_WriteIni(@ScriptDir & "\Data\Account\file" & WinGetTitle($WindowHandle) & ".ini", "General", "SVDAY", 1)
			Sleep(2000)
...
;~ 			EndIf
			...
			ExitToLog()
			AdlibRegister("_checkFB", 5000)
;~

			AdlibRegister("_ClearApp", 16000)
		WEnd
		If ProcessExists(WinGetProcess($WindowHandle)) And $flag = 1 Then
			_WriteIni($iniFile, "General", "Time", @HOUR & " : " & @MIN)
			Sleep(2000)
...
Func _ResetNewDay($fileIni)
	If _ReadIni($fileIni, "General", "Day", 0) <> @MDAY Then
		...
		_WriteIni($fileIni, "General", "BienIVPN", 1)
	...
	Else
		...
		$i = _ReadIni($fileIni, "General", "BienIVPN", 1)
	EndIf
