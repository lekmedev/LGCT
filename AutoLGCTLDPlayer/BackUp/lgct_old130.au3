...
;~ 		Mbox("xx")
;~ 	MsgBox(0,"","xxicon",1)
;~ 		_CtrlClickInGame($WindowHandle, 165, 417)
;~ 		Sleep(100)
;~ 		_CtrlClickInGame($WindowHandle, 165, 417)
;~ 		ControlSend($WindowHandle, "", "", "{BS 12}")
;~ 		Sleep(500)
...
;~ 		_SendKeys($render_hwnd,$infoAcc[2])
	...
	ProcessClose($DOS)
	Sleep(1000)
	Local $iPID = Run(@ComSpec & ' /c tshark.exe -x -r D:test1.pcap"', "C:\Program Files\Wireshark", @SW_HIDE, $STDOUT_CHILD)
	While 1
...
;~ 		_ArrayDisplay($a)
	WEnd
	Sleep(1000)
;~ $aa=_StringBetween('','"cash":',',')
	if Stringlen($aa[1]) < 7 Then
		$wr=$aa[1]
	Else
		$wr=$aa[0]
		EndIf
	FileWriteLine("D:\SumVang120.txt",$infoAcc[1] &" "&$aa[1], " ", "")))
;~ 	FileClose
;~ ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ Sum Vnag
