...
;~ 				AdlibRegister("_ClearApp", 60000)q
				if WinGetTitle($WindowHandle) = "RaoLSV" then

				_ChonSV1($infoAcc[1], $fileIni)
				Else
...
;~ 		--------------------------------------------------------------------------------------------------------
			...
			If _ReadIni(@ScriptDir & "\Data\Account\file.ini", "General", "code", "") <> "" Then
				DoiCode()
			EndIf
			If _curSV() = "SV123" And @MDAY = 11 Then
				_Mail()
			EndIf
...
;~ 	khu vuc sung bai bxh
	If Third($infoAcc, "9t") Or IniRead($iniFile, "General", "SV", 0) = "SV589" Or IniRead($iniFile, "General", "SV", 0) = "SV283" Or IniRead($iniFile, "General", "SV", 0) = "SV281" Or IniRead($iniFile, "General", "SV", 0) = "SV586" Or IniRead($iniFile, "General", "SV", 0) = "SV284" Then
...
Func _ChonSV1($SVer, $fileIni)
	While 1
		...
		If _imgSearch($SVer) = 1 Then
			_CtrlClickImg($x, $y)
			Return
		EndIf
		Sleep(400)
		_ADB_CMD_SLEEP("input swipe 531 401 531 309", $port, 2200)
	WEnd
EndFunc   ;==>_ChonSV1
