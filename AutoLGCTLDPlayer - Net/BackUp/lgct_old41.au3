...
;~ 		----------------------------------------------------------------------------------------------------------- Change SV
		If StringInStr($getDat[1], "SV", 1) Or IniRead($fileIni, "General", "SVDAY", 1) = 1 Then
			OKIF("Day")
			Sleep(500)
			MouseClick("left", 577, 543, 1, 0)
			OKIF("list")
			ChonSV($getDat[1])
			If IniRead($fileIni, "General", "SVDAY", 1) = 1 Then
				IniWrite($fileIni, "General", "SVDAY", 0)
			EndIf
			If StringInStr($getDat[1], "SV", 1) Then
				ContinueLoop
			EndIf
		EndIf
;~ 		--------------------------------------------------------------------------------------------------------
