...
;~ 				mbox(WinGetTitle($WindowHandle))
			ShellExecuteWait("E:\App\AutoLGCTLDPlayer\VanTieuAutoLDPlayer" & WinGetTitle($WindowHandle) & ".au3")
			Exit
...
;~ 	Mbox("1")
	...
	_ADB_CMD("input swipe 63 1274 62 299 510", $port)
	Sleep(1000)
	_ADB_CMD("input swipe 63 512 63 800 1550", $port)
...
;~ 	While 1
;~ 		If $flag = 0 Then Return
;~ 		If _imgSearch("Mat") = 1 Then
;~ 			ExitLoop
;~ 		ElseIf _imgSearch("Mat1") = 1 Then
;~ 			ExitLoop
...
;~ 	Sleep(1000)
	...
	While 1
		If $flag = 0 Then Return
;~ 		_CtrlClickInGame(222, 447);mua mat ca nhan
	...
	While 1
		If $flag = 0 Then Return
;~ 		_CtrlClickInGame(224, 310);mua mat ca nhan
...
;~ 	Mbox("1")
	...
	Sleep(1000)
	_ADB_CMD("input swipe 63 312 63 800 1550", $port)
	_ADB_CMD("input swipe 63 312 63 500 1550", $port)
	Sleep(1100)
...
Func codeTT($doneCode)
	If $iCode < 45 Then
		...
		Local $fileList = @ScriptDir & "\Data\Account\code.txt", $i = 1, $count = _FileCountLines($fileList)
		_ClickUtilSeeImg("iconDoiCode", 24, 412)
		Sleep(109)
;~ 		func here~
While 1
