...
Global $port, $x = 0, $y = 0, $WindowHandle, $tenTK, $iniFile, $iGlobal, $iGlobal1, $aPos, $CoorControl[2], $rsMod
...
Func _ThinhAn($WindowHandle, $infoAcc)
	...
	_CtrlClickInGame($WindowHandle, 143, 60)
...
;~ 	EndIfbig_daisy083
	Run(@ComSpec & ' /c ""C:\Program Files\ImageMagick-7.0.10-Q16\magick" convert "E:\Temp\MemuPhoto\' & $infoAcc[1] & '.jpg" -crop 65x20+520+110  "D:\Vang\Vang\' & _ReadIni($iniFile, "General", "SV", 0) & '\' & $infoAcc[1] & '.jpg""', "", @SW_HIDE)
EndFunc   ;==>_ThinhAn
...
Func _ChonSV($WindowHandle, $SVer, $fileIni)
	...
	Run(@ComSpec & ' /c adb -s 127.0.0.1:5555 shell input swipe 200 860 200 500 100 ', "", @SW_HIDE)
	Sleep(500)
	Run(@ComSpec & ' /c adb -s 127.0.0.1:5555 shell input swipe 200 860 200 500 100 ', "", @SW_HIDE)
	Sleep(500)
	Run(@ComSpec & ' /c adb -s 127.0.0.1:5555 shell input swipe 200 860 200 500 100 ', "", @SW_HIDE)
...
;~ 			Local $SVerfile = _ReadIni($fileIni, "General", "SV", 1)
			...
			If _bmbSearch(WinGetTitle($WindowHandle), $grsv) = 1 Then
				...
				_ADB_CMD_SLEEP("input tap " & $CoorControl[0] & " " & $CoorControl[1] & " ", $port, 550)
...
;~ 			If _imgSearch($WindowHandle, "S181",60, 197,82, 342) = 1
		...
		Sleep(100)
;~
		_ADB_CMD_SLEEP("input swipe 251 522 251 830.5 1500", $port, 3400)
;~ 		846.5 522
		If StringInStr($SVer, "SV", 1) Then
			IniWrite($fileIni, "General", "SV", $SVer)
...
;~ 	If _imgSearch($WindowHandle, "wait") Then
;~ 		Sleep(700)
;~ 		MsgBox(0,$x, $y,1)
...
Func _bmbSearch($SVnoTrim, $SVtrimmed)
	...
	Local $FindBmp = "C:\Users\Admin\Documents\LDPlayer\Misc\S" & $SVtrimmed & ".bmp"
	Local $winPos
;~ 	MsgBox(0,$Source,$FindBmp)
	While 1
		Sleep(10)
		RunWait(@ComSpec & ' /c adb -s 127.0.0.1:5555 shell screencap -p /sdcard/Misc/' & $SVnoTrim & '.bmp', "", @SW_HIDE)
		Local $a1 = TimerInit
		While 1
			MsgBox(0, "", "$a = _BmpImgSearch", 1)
			$a = _BmpImgSearch($Source, $FindBmp, 0, 0, -1, -1, 100, 10)
			If Not @error Then
				Mbox("If Not @error Then", 1)
				$winPos = WinGetClientSize($WindowHandle)
				$CoorControl[0] = ($winPos[0] / 720) * $a[1][0]
				$CoorControl[1] = ($winPos[1] / 1280) * $a[1][1]
				MsgBox(0, "", TimerDiff($a1) - 1, 2)
				Return 1
			EndIf
			MsgBox(0, "", "if timediffa1", 1)
			If TimerDiff($a1) > 1001 Then
				Return
			EndIf
		WEnd
	WEnd
