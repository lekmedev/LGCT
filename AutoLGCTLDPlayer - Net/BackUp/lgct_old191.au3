...
;~ 720x1280 320dpi
;~ _Singleton(
Global $doneNV = 0, $port, $x = 0, $y = 0, $tenTK, $iniFile, $iGlobal1, $iCVH, $aPos, $aPoss, $rsMod, $sFilePathvang, $render_hwnd, $WindowHandle, $flag = 1, $motiec = 0, $flgMT = 0, $iCode, $code
...
;~ 			ShellExecuteWait(@ScriptDir &"\Net" & WinGetTitle($WindowHandle) & " - Copy.au3")
			ExitLoop
...
;~  ChoSale()
;~ChoSaleMat()
	Run(@ComSpec & ' /c "C:\Program Files (x86)\Proxifier\Proxifier.exe" C:\Users\Admin\AppData\Roaming\Proxifier\Profiles\Default1.ppx', "", @SW_HIDE)
	AutoMain1()
	AutomainfullNV()
...
;~ 	_CtrlClickInGame()
	Sleep(600)
	_ClickUtilSeeImg("inphai",132, 72)
;~ 	_OKIF("inphai")
	Sleep(500)
...
Func _Hanhi()
	...
	_OKIF("iconInBang")
	Sleep(300)
	Local $xinbang = 47, $yinbang = 361
	If _imgSearchW("pbang", 1000) = 1 Or _imgSearchW("pbang1", 1000) = 1 Then
		$xinbang = 120
		$yinbang = 365
	EndIf
	_CtrlClickInGame($xinbang, $yinbang)
	_OKIF("inXay") ;c
	Sleep(500)
...
;~ 			Mbox("havenv")
			...
			While 1
				...
				If _imgSearch("nv14") = 1 Or _imgSearch("nv142") = 1 Then
					_CtrlClickImg($x, $y)
;~ 					AdlibRegister("_ClearApp", 15000)
				ElseIf _imgSearch("full130") = 1 Then
					shareFB()
;~ 					ExitLoop
...
;~ 						mbox($iFUllnv)
						Sleep(1000)
						_ClickUtilSeeImg("xemqua", 52 + (49 * $iFUllnv), 77)
						Sleep(300)
						_CtrlClickInGame(249, 190)
						_CtrlClickInGame(249, 190)
					Next
					Return
				Else
;~ 					Mbox("outnv")
			...
			WEnd
		EndIf
		If TimerDiff($countNVtime) > 10000 Then
			Return
		EndIf
	WEnd
EndFunc   ;==>_nv14
...
;~ 				MsgBox(0, $x, $y, 1)
				...
				Sleep(500)
;~ 				_CtrlClickInGame(219, 228) tiec 500
				_CtrlClickInGame(220, 300)
				Return
			EndIf
			If TimerDiff($a) > 400 Then
				ExitLoop
